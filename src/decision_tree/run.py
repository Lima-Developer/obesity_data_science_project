"""Train and evaluate a Decision Tree classifier on the UCI obesity dataset."""
...
def _save_relatorio(
    ...
    lines = [
        ...
        f"Profundidade real: {tree_depth}",
        f"Número de folhas : {n_leaves}",
        "",
...
def _save_matriz_confusao_png(results: dict) -> None:
    ...
    ax.set_title("Matriz de Confusão — Decision Tree", fontsize=13, pad=12)
...
def main() -> None:
    ...
    tree_depth = model.get_tree_depth()
    ...
    separator = "=" * 62
    print(separator)
    print("  RESULTADOS — Decision Tree | UCI Obesity Levels (id=544)")
    print(separator)
    ...
    _save_relatorio(
        results, importances, X_train.shape[0], X_test.shape[0],
        elapsed, tree_depth, n_leaves,
    )
